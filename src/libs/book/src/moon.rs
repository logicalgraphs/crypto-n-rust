...
use book::{
   ...
   num::estimate::functional_tests::runoff as ne,
   num::percentage::functional_tests::runoff as np,
   ...
   types::dyadic::functional_tests::runoff as td,
...
fn main() -> ErrStr<()> {
   ...
   let t3 = "list_utils rest_utils matrix_utils num::percentage types::dyadic";
   let tests = format!("{t1} {t2} {t3}");
   let _ = collate_results("book",
      & mut mk_tests(&tests,
            vec![mk_sync(d), mk_sync(f), mk_sync(stri), mk_sync(u), mk_sync(ne),
                 mk_sync(p), mk_sync(n), mk_sync(t), mk_sync(stre),
                 mk_sync(tu), mk_sync(l), mk_async(r()), mk_sync(m),
                 mk_sync(np), mk_sync(td)]))?;
